...
/// Configurable mock PrinterManager for tests.
...
/// Offline mock — all printers report Offline.
...
fn setup() -> (NativeMessageHandler, Arc<SqlitePrintJobRepository>, Arc<TestPrinterManager>) {
    ...
    let handler = NativeMessageHandler::new(
        ...
        Arc::new(MetricsCollector::new(
            arc_conn.clone(),
            Arc::new(SqliteQueueManager::new(arc_conn)),
        )),
    );

    (handler, job_repo, printer_manager)
}
...
fn test_print_batch_end_to_end() {
    let (handler, _, printer_manager) = setup();
    printer_manager.add_online_printer("HP_Integration");
...
fn test_get_status_end_to_end() {
    let (handler, job_repo, printer_manager) = setup();
    printer_manager.add_online_printer("HP_Status");
...
fn test_cancel_job_end_to_end() {
    let (handler, job_repo, printer_manager) = setup();
    printer_manager.add_online_printer("HP_Cancel");
...
fn test_list_printers_end_to_end() {
    let (handler, _, printer_manager) = setup();
    printer_manager.add_online_printer("TestPrinter1");

    let resp = simulate_exchange(&handler, r#"{"command":"list_printers"}"#);
    assert_eq!(resp["success"], true);
    let printers = resp["data"]["printers"].as_array().unwrap();
    assert_eq!(printers.len(), 1);
    assert_eq!(printers[0]["name"], "TestPrinter1");
}
...
fn test_cancel_completed_job_returns_invalid_state() {
    let (handler, job_repo, printer_manager) = setup();
    printer_manager.add_online_printer("HP_Completed");
...
fn setup_with_offline_printer() -> NativeMessageHandler {
    let mut conn = Connection::open_in_memory().unwrap();
    run_migrations(&mut conn).unwrap();
    let arc_conn = Arc::new(Mutex::new(conn));
...
fn test_print_batch_offline_printer_returns_not_available() {
    let handler = setup_with_offline_printer();
