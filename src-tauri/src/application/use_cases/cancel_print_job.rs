...
use crate::application::ports::{EventStore, TempFilePort};
...
impl CancelPrintJobUseCase {
    ...
    pub fn execute(&self, request: CancelPrintJobRequest) -> Result<(), Error> {
        tracing::info!(
        ...
        job.cancel().map_err(|e| match e {
            PrintJobError::CannotCancelCompleted => Error::CannotCancelCompleted {
                job_id: job_id.to_string(),
            },
            PrintJobError::CannotCancelFailed => Error::CannotCancelFailed {
                job_id: job_id.to_string(),
            },
            PrintJobError::CannotCancelCancelled => Error::CannotCancelCancelled {
                job_id: job_id.to_string(),
            },
            _ => Error::DomainRuleViolation {
                reason: format!("{:?}", e),
            },
        })?;
